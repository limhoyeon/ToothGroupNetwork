...
parser.add_argument('--input_dir_path', type=str, default="G:/tooth_seg/main/all_datas/chl/3D_scans_per_patient_obj_files", help = "input directory path that contain obj files.")
parser.add_argument('--split_txt_save_path', type=str, default=".", help = "split txt path.")
...
all_path_ls = []
for item_path in glob.glob(os.path.join("datasets/data_chl_fps", "*_sampled_points.npy")):
...
test_num = int(args.split_ratio[0] * len(all_path_ls))

train_ls = all_path_ls[train_num]
val_ls = all_path_ls[train_num:train_num+val_num]
test_ls = all_path_ls[train_num+val_num:]

f_train = open(os.path.join(args.split_txt_save_path, f"base_name_train_fold.txt"), 'w')
f_val = open(os.path.join(args.split_txt_save_path, f"base_name_val_fold.txt"), 'w')
f_test = open(os.path.join(args.split_txt_save_path, f"base_name_test_fold.txt"), 'w')
