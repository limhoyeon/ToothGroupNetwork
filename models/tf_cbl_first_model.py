...
class TfCblFirstModule(torch.nn.Module):
...
class TfCblFirstModel(BaseModel):
    ...
    def get_loss(self, offset_1, sem_1, offset_2, sem_2, gt_seg_label_1, input_coords):
        ...
        if self.config["tr_set"]["only_tooth"]:
            tooth_class_loss_1 = tsg_ins_cent_loss.tooth_class_loss(sem_1, half_seg_label, label_smoothing=self.config["tr_set"]["label_smoothing"], weight=self.config["tr_set"]["first_cr_weight"])
            return {
    ...
    def step(self, batch_idx, batch_item, phase):
        ...
        if phase == "train":
            if not self.config["tr_set"]["only_tooth"]:
    ...
    def infer(self, batch_idx, batch_item, **options):
        ...
        #gu.print_3d(gu.np_to_pcd_with_label(full_labeled_points_1))
        ...
        if True:
            sem_1_gt_mask_acc = mu.cal_metric(gt_seg_label.reshape(-1)+1, full_labeled_points_3[:,3], gt_seg_label.reshape(-1)+1, is_half=True)[3]
            self.sem_1_gt_mask_acc += sem_1_gt_mask_acc
            if sem_1_gt_mask_acc<0.99:
                a=1
        ...
        if False:
            ...
            #sem accuracy
            sem_1_acc = mu.cal_metric(gt_seg_label.reshape(-1)+1, results["sem_1"]["full_labeled_points"][:,3], gt_seg_label.reshape(-1)+1, is_half=True)[3]
        ...
        if False:
            ...
            gu.print_3d(gu.np_to_pcd_with_label(results["sem_1"]["full_labeled_points"]))
            gin_infer_miss_points = results["sem_1"]["full_labeled_points"].copy()
            half_gt_seg_label = gt_seg_label.reshape(-1)+1
            half_gt_seg_label[half_gt_seg_label>9] -= 8 
            gin_infer_miss_points_mask = (results["sem_1"]["full_labeled_points"][:,3]) != (half_gt_seg_label)
            gin_infer_miss_points[gin_infer_miss_points_mask, 3] = 1
            gin_infer_miss_points[np.invert(gin_infer_miss_points_mask), 3] = 0
            t = gu.np_to_pcd_with_label(gin_infer_miss_points)
            t.estimate_normals()
            gu.print_3d(t)
        return
